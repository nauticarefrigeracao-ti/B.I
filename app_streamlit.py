...
def render_interactive_table(df, table_id='tbl'):
    ...
    if '#' in df2.columns and 'order id' in cols_lower:
        ...
        for i, oid in enumerate(df2[oid_col].fillna('').astype(str), start=1):
            ...
            link = f'<a href="#" class="open-link" data-order="{html.escape(oid)}" title="Copiar e instruir para abrir detalhe">{i}</a>'
    ...
    if 'order id' in cols_lower:
        ...
        df2[order_col_name] = df2[order_col_name].fillna('').astype(str).apply(
            lambda oid: f'<a href="https://www.mercadolivre.com.br/vendas/{html.escape(oid)}/detalhe" target="_blank" rel="noopener noreferrer">{html.escape(oid)}</a>' if oid else ''
        )
